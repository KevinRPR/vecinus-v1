# Task List (Premium Quality)

## Done
- [x] Create docs/QUALITY_BAR.md
- [x] Create docs/JOURNEY_MAP.md
- [x] Create docs/UI_MOTION_SPEC.md
- [x] Create docs/MULTI_STAGE_PLAN.md

## Plan (Stages)

### Stage 0 - Baseline and measurement
- [ ] Define event schema for the 3 journeys.
- [ ] Instrument events and funnels (web + mobile).
- [ ] Build a small metrics dashboard (success rate, time to value, errors).
- [ ] QA matrix: low/mid/high devices + Chrome/Safari.

### Stage 1 - Premium UX polish
- [x] Fix mojibake strings and copy consistency.
- [x] Unify icon family and spacing across web + mobile.
- [x] Add skeletons and empty states to critical lists.
- [x] Add accessibility labels + 44dp targets for icon-only.
- [x] Apply motion spec to key screens (transitions + feedback).

### Stage 2 - Reliability and performance
- [x] Improve report payment flow (prefill, validation, idempotency).
- [x] Optimize list rendering and caching (large data sets).
- [x] Reduce heavy blur/shadows on low-end devices.

### Stage 3 - Trust and secure sessions
- [x] Refresh token end-to-end (server + client).
- [ ] Secure unlock flow (biometric/PIN + fallback).

### Stage 4 - Delight and expansion
- [ ] Push notifications for novedades and pagos.
- [ ] Digest/summary for novedades.
- [ ] Downloadable receipts in payment history.
